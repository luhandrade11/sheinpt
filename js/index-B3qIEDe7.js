...
/* SHEIN · v6 — design fiel ao app nativo Shein
   · branco puro, divisores em vez de cards, imagem edge-to-edge
   · pill saldo branco com border, back só ícone, botão preto radius 2px */
;(function () {
  'use strict';
  ...
  var CSS = [
    ...
    /* v2 mantido (toasts, step rows, hero, marquee) */
    ...
    /* === v6 · Tela de avaliação (Shein-style nativo) === */
    ...
    /* Header — sticky, branco puro, divisor 1px */
    ...
    /* Pill saldo — branca, número preto bold, accent verde apenas no símbolo */
    ...
    /* Main · sem padding lateral (edge-to-edge) */
    ...
    /* Imagem — edge-to-edge, sem radius */
    ...
    /* Secções (não-cards) — divisores 1px */
    ...
    /* Estrelas — menores e cinza mais escuro quando vazias */
    ...
    /* Textarea — border subtil, radius pequeno */
    ...
    /* Submit — preto absoluto, radius 2px, largura quase total */
    ...
    /* Loading overlay */
    '.se-ld{position:fixed;inset:0;z-index:9100;background:rgba(255,255,255,.82);-webkit-backdrop-filter:saturate(160%) blur(10px);backdrop-filter:saturate(160%) blur(10px);display:grid;place-items:center;animation:seFadeUp .2s ease;padding:20px}',
    '.se-ld.hidden{display:none}',
    '.se-ld-c{background:#fff;border-radius:2px;padding:28px 28px;max-width:300px;text-align:center;border:1px solid #eee;box-shadow:0 24px 50px -8px rgba(0,0,0,.16)}',
    '.se-spin{width:32px;height:32px;border:2.5px solid #eee;border-top-color:#000;border-radius:50%;margin:0 auto 14px;animation:seSpin .8s linear infinite}',
    '.se-ld-t{font-size:14px;font-weight:700;margin:0 0 6px;letter-spacing:-.01em;color:#000}',
    '.se-ld-s{font-size:11.5px;color:rgba(0,0,0,.55);margin:0;letter-spacing:-.005em}',
    ...
    /* Reduced motion */
    '@media(prefers-reduced-motion:reduce){.shein-toast-in,.shein-toast-out,.shein-toast-dot,.shein-pop-in,.shein-step-row.shein-revealed,.se-prog-fill,.se-spin,.se-star.active svg,.se-bal.bump{animation:none!important;transition:none!important}}'
  ].join('');
  ...
  /* Toasts / step rows / hero — idêntico ao v5 */
  ...
  function eur2(n){try{var s=new Intl.NumberFormat('pt-PT',{style:'currency',currency:'EUR',minimumFractionDigits:2,maximumFractionDigits:2}).format(n);return s.replace(/ ?€$/,'').replace(/^€\s?/,'').trim();}catch(e){return n.toFixed(2).replace('.',',');}}
  ...
  function submit(){
    ...
    var ld=document.getElementById('se-ld');ld.classList.remove('hidden');
    setTimeout(function(){
      var balEl=document.getElementById('se-bal-val');
      var balWrap=document.querySelector('.se-bal');
      var from=ev.balance,to=ev.balance+rew;
      animateCounter(balEl,from,to,900);
      if(balWrap){balWrap.classList.add('bump');setTimeout(function(){balWrap.classList.remove('bump');},500);}
      ev.balance=to;
      ...
      if(ev.idx>=TOTAL){
        ...
        setTimeout(function(){document.getElementById('se-done').classList.remove('hidden');},450);
      }else renderLook();
    },1500);
  }
  ...
  function buildOverlay(){
    ...
    ov.innerHTML=
      ...
      '<div id="se-ld" class="se-ld hidden">'+
        '<div class="se-ld-c"><div class="se-spin"></div><p class="se-ld-t">A validar com os servidores Shein…</p><p class="se-ld-s">A confirmar a tua opinião com a base de dados</p></div>'+
      '</div>'+
  ...
  function init(){injectCSS();markStepRows();observeRows();applyHeroEnter();cacheImgs();patchHistory();checkRoute();startToasts();watch();}
